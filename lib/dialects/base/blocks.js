...
var termKeys = ['select', 'query', 'field', 'value', 'func', 'expression', 'case'];
...
module.exports = function(dialect) {
	...
	dialect.blocks.add('fields', function(params) {
		...
		} else {// If fields is object: {a: 'u', b: {table: 't', alias: 'c'}}
	...
	dialect.blocks.add('case', function (params) {
		const { when, elseValue } = params.case;
		
		const whenCases =
			when.map(w => {
				const cond = dialect.buildCondition({ value: w['condition'], defaultFetchingOperator: '$value' });
				const value = w['then'];
				return `when ${cond} then ${value}`;
			}).join(' ');
		
		return `case ${whenCases} else ${elseValue} end`;
	});
	...
	dialect.blocks.add('where', function(params) {
		var result = dialect.buildBlock('condition', params.where);
